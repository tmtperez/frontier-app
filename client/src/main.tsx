    ...
    <header className="topbar border-b border-slate-200">
      <div className="container-7xl mx-auto px-4 py-3 flex items-center justify-between">
        ...
        <nav className="flex gap-1 font-semibold">
          {onLoginPage ? (
            // Only show Login button if on login page
...
function LoginPage() {
  ...
  // start blank (no demo defaults)
  const [email, setEmail] = React.useState('')
  const [password, setPassword] = React.useState('')
  const [err, setErr] = React.useState<string>('')

  // stronger autofill prevention: real inputs are readOnly until focus
  ...
  return (
    <div className="max-w-md mx-auto mt-10 card p-6">
      ...
      <form
        ...
        autoComplete="off"
      >
        {/* Decoy inputs (off-screen) so password managers target these instead */}
        <div
          aria-hidden="true"
          style={{
            position: 'absolute',
        ...
        <div>
          ...
          <input
            ...
            type="email"
            name="login-email"                 // non-standard name
            value={email}
            onChange={e => setEmail(e.target.value)}
            placeholder="you@company.com"
            autoComplete="new-email"           // non-standard token
            readOnly={emailRO}
            onFocus={() => setEmailRO(false)}  // unlock on real user focus
          />
        </div>
        <div>
          ...
          <input
            className="input w-full"
            type="password"
            name="login-password"              // non-standard name
            value={password}
            onChange={e => setPassword(e.target.value)}
            placeholder="••••••••"
            autoComplete="new-password"        // non-standard token
            readOnly={passRO}
            onFocus={() => setPassRO(false)}   // unlock on real user focus
          />
        </div>
